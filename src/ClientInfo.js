...
const App = () => {
  ...
  const handleAddUser = async () => {
    ...
    try {
      await axios.post(`http://127.0.0.1:8000/api/user-search/`, {
        id: newUser.id,
...
<Modal open={isAddModalOpen} onClose={() => setIsAddModalOpen(false)}>
  ...
  >
    ...
    <TextField
      ...
      onChange={(e) => setNewUser((prev) => ({ ...prev, registration_date: e.target.value }))}
    ...
    <Box display="flex" justifyContent="center" mt={3}>
      ...
      <Button variant="outlined" onClick={() => setIsAddModalOpen(false)} sx={{ marginLeft: 2 }}>
        Cancelar
      </Button>
    </Box>
  </Box>
...
<Modal open={isEditModalOpen} onClose={() => setIsEditModalOpen(false)}>
  ...
  >
    ...
    <TextField
      ...
      label="Fecha de Registro"
      type="date"
      value={selectedUser?.registration_date?.split("T")[0] || ""} // Remove time if present
      onChange={(e) =>
        setSelectedUser((prev) => ({
          ...prev,
          registration_date: e.target.value, // Ensure correct date format
        }))
      }
      InputLabelProps={{ shrink: true }}
    />
    ...
    />
    <Box display="flex" justifyContent="center" mt={3}>
      <Button variant="contained" color="success" onClick={handleEditUser}>
        Guardar
      </Button>
      <Button variant="outlined" onClick={() => setIsEditModalOpen(false)} sx={{ marginLeft: 2 }}>
        Cancelar
      </Button>
    </Box>
  </Box>
